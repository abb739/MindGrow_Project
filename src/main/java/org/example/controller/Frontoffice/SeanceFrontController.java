...
public class SeanceFrontController {
    ...
    private Node createFlashcard(Seance s) {
        ...
        if (s.getImage() != null && !s.getImage().isEmpty()) {
            try {
                iv.setImage(new Image(new File(s.getImage()).toURI().toString()));
            } catch (Exception e) {
            }
        }
        iv.setFitWidth(280);
        ...
        date.setTextFill(Color.web("#0b7a8f"));
        date.setStyle("-fx-font-weight: bold;");

        Label lieu = new Label("📍 " + s.getLieu());
        lieu.setTextFill(Color.web("#7f8c8d"));
        lieu.setWrapText(true);

        Label cap = new Label("👥 Capacité: " + s.getCapacite() + " personnes");
        cap.setTextFill(Color.web("#27ae60"));

        Label desc = new Label(s.getDescription());
        desc.setWrapText(true);
        desc.setTextFill(Color.web("#34495e"));
        desc.setMaxHeight(60);

        Button btnReserve = new Button("🎟 Réserver");
        btnReserve.setMaxWidth(Double.MAX_VALUE);
        btnReserve.setStyle(
                "-fx-background-color: #a2d5ab; -fx-text-fill: #4a4a4a; -fx-font-weight: bold; -fx-background-radius: 15; -fx-cursor: hand; -fx-padding: 8 20;");
        ...
        card.setOnMouseEntered(e -> card.setStyle(
                "-fx-background-color: white; -fx-background-radius: 20; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.1), 20, 0, 0, 8); -fx-border-color: #a2d5ab; -fx-border-radius: 20; -fx-border-width: 1; -fx-translate-y: -5;"));
